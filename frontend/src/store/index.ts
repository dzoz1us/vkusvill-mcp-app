...
interface AppState {
  ...
  setFormField: <K extends keyof OnboardingParams>(key: K, value: OnboardingParams[K]) => void;
  resetForm: () => void;
...
export const useAppStore = create<AppState>((set, get) => ({
  // Initial form state
  form: { ...defaultForm },
  setFormField: (key, value) => set(state => ({
    form: { ...state.form, [key]: value }
  })),
  resetForm: () => set({ form: { ...defaultForm, days: [], preferences: [], appliances: [] } }),
  ...
  generatePlan: async () => {
    ...
    try {
      ...
      set({ planState: 'error', planError: 'Не удалось создать план питания. Попробуйте ещё раз.' });
    }
  },
  ...
  replaceMeal: async (planId: string, mealId: string) => {
    set({ replaceState: 'loading', replacingMealId: mealId, replaceError: null });
    try {
  ...
  createCart: async (planId: string) => {
    ...
    try {
      ...
      set({ cartState: 'error', cartError: 'Не удалось создать корзину. Попробуйте позже.' });
    }
  },
  ...
  // Clear cart
  clearCart: () => set({ cartResponse: null, cartState: 'idle', cartError: null }),
}));
