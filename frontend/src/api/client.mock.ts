...
function generateRecipe(
  ...
  const startIdx = Math.floor(
    Math.random() * (allIngredients.length - ingredientCount),
  );
  const ingredients = allIngredients.slice(
    startIdx,
    startIdx + ingredientCount,
  );
...
function generateGroceryList(plan: MealPlan): GroceryList {
  ...
  plan.meals.forEach((meal) => {
    ...
    fullRecipe.ingredients.forEach((ing: Ingredient) => {
      ...
      if (existing) {
        ...
        allIngredientsMap.set(ing.name, {
          name: ing.name,
  ...
  const statuses: ("matched" | "requires_review" | "not_found")[] = [
    "matched",
    "matched",
    "matched",
    "requires_review",
  ...
  const items: GroceryItem[] = Array.from(allIngredientsMap.values()).map(
    (ing, i) => {
      const packageQty =
        ing.unit === "г"
          ? 500
      ...
      const pricePerPackage =
        ing.unit === "г"
          ? 89 + Math.floor(Math.random() * 100)
          : ing.unit === "мл"
            ? 59 + Math.floor(Math.random() * 60)
            : 29 + Math.floor(Math.random() * 150);
      const itemPrice = pricePerPackage * packageCount;
      totalCost += itemPrice;

      const status =
        i < allIngredientsMap.size * 0.7
          ? "matched"
          : i < allIngredientsMap.size * 0.9
            ? "requires_review"
            : "not_found";
...
export const api = {
  ...
  async getRecipe(id: string): Promise<Recipe> {
    ...
    return generateRecipe(
      ["breakfast", "lunch", "dinner"][Math.floor(Math.random() * 3)] as
  ...
  async replaceMeal(
    ...
    const newRecipe = generateRecipe(
      mealTypes[Math.floor(Math.random() * 3)],
